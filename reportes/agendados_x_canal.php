...
while($gt = mysql_fetch_assoc($tipos_canales)){
	...
	$objPHPExcel->getProperties()->setCreator("EPICMEDIA (www.epicmedia.pro)")
								 ...
								 ->setTitle("REPORTE DE PACIENTES AGENDADOS POR CANAL")
								 ->setSubject("Pacientes agendados por canal del ".$fecha1." al ".$fecha2)
	...
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue('A3','REPORTE DE PACIENTES AGENDADOS POR CANAL');
...
for($y = 0; $y < count($consultorios); $y++){
    ...
    $tit_1 = $str;
	$str = ++$str;
	for($z = 0; $z < count($canales); $z++){
		$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.'8',$canales[$z]['nom']);      	
	    $titulos[] = $str;
		$str = ++$str;
	}
	...
	//CONSULTAMOS POR CADA UNO DE LOS DIAS QUE HAY EN LA LISTA				
		...
		for($x = 0; $x < count($canales); $x++){
			...
			$pacientes_clinica = mysql_query("SELECT citas.id_paciente,pacientes.id_canal FROM citas 
	...
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.$cuenta,'PACIENTES QUE ASISTIERON');
    $cuenta++;
    $nuevos = $cuenta;
for($y = 0; $y < count($consultorios); $y++){
	$id_clinica = $consultorios[$y]['id'];
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.$cuenta,$consultorios[$y]['nom']);
    $cuenta++;
    $objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.$cuenta,'FECHA');    
    $tit_1 = $str;
    $str = ++$str;
    for($z = 0; $z < count($canales); $z++){
		$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.$cuenta,$canales[$z]['nom']);      	
	    $titulos[] = $str;
		$str = ++$str;
	}
	 $cuenta++;
	...
	for($i = 0;$i < count($dias);$i++){				
		...
		for($x = 0; $x < count($canales); $x++){
			...
			//SACAMOS LOS PACIENTES QUE FUERON ATENDIDOS EN LA CLINICA ESE DIA 
			$pacientes_clinica = mysql_query("SELECT DISTINCT(citas.id_cita),pacientes.id_canal FROM citas 
											JOIN pacientes ON pacientes.id_paciente = citas.id_paciente
											WHERE id_clinica = '$id_clinica' AND DATE(fecha_hora) = '$dia_actual' AND citas.atendida = 1 AND pacientes.id_canal='$id_canal' AND citas.tipo=1 AND citas.activo=1");
...
for($y = 0; $y < count($consultorios); $y++){
    ...
    for($z = 0; $z < count($canales); $z++){
		$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($titulos[$z].$cuenta,$canales[$z]['nom']);      	
	}
...
*/	
	...
	// Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle('Pacientes Agendados Canal');
	...
	header('Content-Disposition: attachment;filename="Reporte_de_agendados_canal_DENTIXA-CRM.xlsx"');
