...
$tipos_metodos = mysql_query("SELECT * FROM books_metodo_pago WHERE activo = 1");
$metodos = array();
while($gt = mysql_fetch_assoc($tipos_metodos)){
	array_push($metodos, array("id"=>$gt['id_metodo_pago'],"nom"=>$gt['metodo_pago']));
}
	...
	$objPHPExcel->getProperties()->setCreator("EPICMEDIA (www.epicmedia.pro)")
								 ...
								 ->setTitle("REPORTE DE VENTAS POR MÉTODO DE PAGO")
	...
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue('A3','REPORTE DE VENTAS POR MÉTODO DE PAGO');
...
while($consultorio = mysql_fetch_assoc($consultorios)){
    ...
    $tit_1 = $str;
	$str = ++$str;
	for($y = 0; $y < count($metodos); $y++){
		$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.'7',$metodos[$y]['nom']);      	
	    $titulos[] = $str;
		$str = ++$str;
	}
	$str = ++$str;
	...
	for($i = 0;$i < count($dias);$i++){				
		...
		for($x = 0; $x < count($metodos); $x++){
			...
			while($cc = mysql_fetch_assoc($cuentas_clinica)){
				...
				$dineros = mysql_fetch_assoc(mysql_query("SELECT SUM(monto) AS monto FROM books_ingresos WHERE id_cuenta='$id_cuenta' AND id_metodo_pago='$id_metodo' AND fecha_ingreso='$dia_actual' AND activo = 1"));
	...
	header('Content-Disposition: attachment;filename="Reporte_de_ventas_metodos_DENTIXA-CRM.xlsx"');
