...
while($gt = mysql_fetch_assoc($tipos_canales)){
	...
	$objPHPExcel->getProperties()->setCreator("EPICMEDIA (www.epicmedia.pro)")
								 ...
								 ->setTitle("REPORTE DE VENTAS POR CANAL")
	...
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue('A3','REPORTE DE VENTAS POR CANAL');
...
while($consultorio = mysql_fetch_assoc($consultorios)){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.'7','VENTAS DE PACIENTES ATENDIDOS, PROVENIENTES DE:');
    $objPHPExcel->setActiveSheetIndex(0)
	...
	for($y = 0; $y < count($canales); $y++){
	    ...
	    $titulos[] = $str;
		$str = ++$str;
		array_push($formulas_t,"=");
		array_push($formulas_nt,"=");
	}
	$str = ++$str;
	
	//CREAR EL ARRAY DE LOS DIAS PARA PODER IR AGREGANDOLOS AL DIA QUE CORRESPONDE
	for($i = 0;$i < count($dias);$i++){	
		array_push($f_totales,$formulas_t);
		array_push($f_totales_n,$formulas_nt);
	}
	...
	for($i = 0;$i < count($dias);$i++){				
		...
		for($x = 0; $x < count($canales); $x++){
			...
			$pacientes_clinica = mysql_query("SELECT DISTINCT(consultas.id_paciente),pacientes.id_canal FROM consultas 
											JOIN pacientes ON pacientes.id_paciente = consultas.id_paciente
											WHERE id_clinica = '$id_clinica' AND DATE(fecha_hora) = '$dia_actual' AND pacientes.id_canal='$id_canal' AND consultas.activo=1");
			
			//CICLO PARA SACAR CADA UNA DE LAS CONSULTAS DEL USUARIO DE ESE DIA E IR SUMANDO CUANTO FUE EL IMPORTE
			while($paciente = mysql_fetch_assoc($pacientes_clinica)){
				$id_paciente = $paciente['id_paciente'];
				
				$dinero = mysql_query("SELECT SUM(monto) AS monto FROM books_ingresos 
											JOIN consultas ON consultas.id_consulta = books_ingresos.id_consulta
											WHERE DATE(consultas.fecha_hora) = '$dia_actual' AND consultas.id_paciente='$id_paciente' AND consultas.id_clinica = '$id_clinica' AND books_ingresos.activo=1");
				$dinero = mysql_fetch_assoc($dinero);
				$dinero_dia += $dinero['monto'];
			}

			
			$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($titulos[$x].$cuenta,$dinero_dia); 
            
            $f_totales[$i][$x] .= $titulos[$x].$cuenta."+";	
	...
	$fin_1 = $cuenta;
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($tit_1.$cuenta,'CLÍNICA: '.dameClinica($id_clinica));
    $cuenta++;
    $fin_2 = $cuenta;
    $objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($tit_1.$cuenta,'VENTAS DE PACIENTES NUEVOS ATENDIDOS, PROVENIENTES DE:');
    $cuenta++;
    $fin_3 = $cuenta;
    $objPHPExcel->setActiveSheetIndex(0)
    ...
    for($y = 0; $y < count($canales); $y++){
		$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($titulos[$y].$cuenta,$canales[$y]['nom']);      	
	}
	 $cuenta++;
	 $nuevos = $cuenta;
	...
	for($i = 0;$i < count($dias);$i++){				
		...
		for($x = 0; $x < count($canales); $x++){
			...
			//SACAMOS LOS PACIENTES QUE FUERON ATENDIDOS EN LA CLINICA ESE DIA 
			$pacientes_clinica = mysql_query("SELECT DISTINCT(consultas.id_paciente) AS id_paciente,pacientes.id_canal FROM consultas 
											JOIN pacientes ON pacientes.id_paciente = consultas.id_paciente
											WHERE id_clinica = '$id_clinica' AND DATE(fecha_hora) = '$dia_actual' AND pacientes.id_canal='$id_canal' AND consultas.activo=1");
			
			while($paciente = mysql_fetch_assoc($pacientes_clinica)){
				$id_paciente = $paciente['id_paciente'];
				$check = mysql_num_rows(mysql_query("SELECT * FROM consultas WHERE id_paciente = '$id_paciente' AND activo=1"));
				if($check == 1){
					$dinero = mysql_query("SELECT monto FROM books_ingresos 
											JOIN consultas ON consultas.id_consulta = books_ingresos.id_consulta
											WHERE DATE(consultas.fecha_hora) = '$dia_actual' AND consultas.id_paciente='$id_paciente' AND consultas.id_clinica = '$id_clinica' AND books_ingresos.activo=1");
					$dinero = mysql_fetch_assoc($dinero);
					$dinero_dia += $dinero['monto'];
            ...
            $f_totales_n[$i][$x] .= $titulos[$x].$cuenta."+";	
...
// 	AQUI VAN LOS TOTALES
	...
	for($i = 0;$i < count($dias);$i++){
		...
		for($x = 0; $x < count($canales); $x++){
		    $objPHPExcel->setActiveSheetIndex(0)
			            ->setCellValue($titulos[$x].$cuenta, '='.substr($f_totales[$i][$x],0,-1));			
	    }
	    $cuenta++;
	}
	...
	for($i = 0;$i < count($dias);$i++){
		...
		for($x = 0; $x < count($canales); $x++){
		    $objPHPExcel->setActiveSheetIndex(0)
			            ->setCellValue($titulos[$x].$cuenta, '='.substr($f_totales_n[$i][$x],0,-1));			
	    }
	    $cuenta++;
	}
	...
	$objPHPExcel->getActiveSheet()->getStyle('A'.$fin_1.':'.$str.$fin_1)->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$fin_1.':'.$str.$fin_1)->getFont()->setSize(12);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$fin_2.':'.$str.$fin_2)->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$fin_2.':'.$str.$fin_2)->getFont()->setSize(12);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$fin_3.':'.$str.$fin_3)->getFont()->setBold(true);
	$objPHPExcel->getActiveSheet()->getStyle('A'.$fin_3.':'.$str.$fin_3)->getFont()->setSize(12);
	...
	// Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle('Ventas por Canal');
	...
	header('Content-Disposition: attachment;filename="Reporte_de_ventas_canales_DENTIXA-CRM.xlsx"');
