...
$tipos_gastos = mysql_query("SELECT * FROM books_tipos_gasto WHERE activo = 1");
$gastos = array();
while($gt = mysql_fetch_assoc($tipos_gastos)){
	array_push($gastos, array("id"=>$gt['id_tipo_gasto'],"nom"=>$gt['cuenta_gasto']));
}
	...
	$objPHPExcel->getProperties()->setCreator("EPICMEDIA (www.epicmedia.pro)")
								 ...
								 ->setTitle("REPORTE DE TODOS LOS GASTOS")
	...
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue('A3','REPORTE DE TODOS LOS GASTOS');
...
while($consultorio = mysql_fetch_assoc($consultorios)){
	...
	for($y = 0; $y < count($gastos); $y++){
		$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.'7',$gastos[$y]['nom']);      	
	    $titulos[] = $str;
		$str = ++$str;
	}
	...
	for($i = 0;$i < count($dias);$i++){				
		...
		//GASTOS POR DIA POR CADA UNO DE LOS TIPOS
		for($x = 0; $x < count($gastos); $x++){
			$dinero_dia = 0;
			$id_gasto = $gastos[$x]['id'];
			
			//SACAMOS LAS CUENTAS DE LA CLINICA
			$cuentas_clinica = mysql_query("SELECT * FROM books_cuentas WHERE id_empresa = '$id_clinica'");
			
			while($cc = mysql_fetch_assoc($cuentas_clinica)){
				$id_cuenta = $cc['id_cuenta'];
				$dineros = mysql_fetch_assoc(mysql_query("SELECT SUM(monto) AS monto FROM books_gastos WHERE id_cuenta='$id_cuenta' AND id_tipo_gasto='$id_gasto' AND fecha_gasto='$dia_actual' and activo=1"));
				$dinero_dia += $dineros['monto'];
			}

			
			$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($titulos[$x].$cuenta,money_format('%i',$dinero_dia)); 
    ...
    for($x = 0; $x < count($gastos); $x++){
	    $objPHPExcel->setActiveSheetIndex(0)
		            ->setCellValue($titulos[$x].$cuenta, '=SUM('.$titulos[$x].'8:'.$titulos[$x].''.$celdas_total.')');
    }
	...
	$objPHPExcel->getActiveSheet()->getHeaderFooter()->setOddHeader('&L&BReporte de Gastos&RDentixa CRM &D');
	...
	// Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle('Todos los Gastos');
	...
	header('Content-Disposition: attachment;filename="Reporte_de_gastos_completo_DENTIXA-CRM.xlsx"');
