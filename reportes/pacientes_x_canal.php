...
while($gt = mysql_fetch_assoc($tipos_canales)){
	...
	$objPHPExcel->getProperties()->setCreator("EPICMEDIA (www.epicmedia.pro)")
								 ...
								 ->setTitle("REPORTE DE PACIENTES POR CANAL")
	...
	$objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue('A3','REPORTE DE PACIENTES POR CANAL');
...
while($consultorio = mysql_fetch_assoc($consultorios)){
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($str.'7','PACIENTES ATENDIDOS, PROVENIENTES DE:');
    $objPHPExcel->setActiveSheetIndex(0)
    ...
    $cuenta++;
    $objPHPExcel->setActiveSheetIndex(0)
            	->setCellValue($tit_1.$cuenta,'PACIENTES NUEVOS ATENDIDOS, PROVENIENTES DE:');
    $cuenta++;
    $objPHPExcel->setActiveSheetIndex(0)
...
// 	AQUI VAN LOS TOTALES
	...
	// Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle('Pacientes por Canal');
	...
	header('Content-Disposition: attachment;filename="Reporte_de_pacientes_canales_DENTIXA-CRM.xlsx"');
