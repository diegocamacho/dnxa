<?
if($s_tipo==3):
	$id_clinica=$s_id_clinica;
	$getSta="books_cuentas.id_empresa=".$id_clinica;
	$getSta2="id_clinica=".$id_clinica;
else:
	if($_GET['id']):
		$id_clinica=$_GET['id'];
		$getSta="books_cuentas.id_empresa=".$id_clinica;
...
if(!$tipo_web):
	...
	//Ingresos
	$sql="SELECT books_ingresos.*, books_clientes.cliente,books_metodo_pago.metodo_pago,books_tipos_ingreso.cuenta_ingreso,id_cuenta_emisora,id_consulta,books_ingresos.activo,referencia FROM books_ingresos 
...
//$ingresos=dameIngresos($id_clinica);
//$egresos=dameEgresoso($id_clinica);
	        ...
	        <div class="portlet light portlet-fit ">
	            <div class="portlet-title">
	                <div class="caption">
	                    ...
	                    <span class="caption-subject font-dark sbold uppercase">Transacciones de <?=$ft['empresa'] ?> (<?=$ft['alias'] ?>) </span>
	                    <div class="caption-desc font-grey-cascade"> <?=$subtitulo?> - Vista de operaciones en todas las cuentas. </div>
	                </div>
	                <div class="actions">
		                <a class="btn red-thunderbird hidden-print print-btn" onclick="javascript:window.print();">Imprimir</a>&nbsp;&nbsp;
		                <? if($fecha1): ?>
		                <a href="?Modulo=TransaccionesClinica&id=<?=$id_clinica?>" class="btn btn-sm red-thunderbird hidden-print"> Últimos 30 movimientos </a>&nbsp;&nbsp;
		                <? endif; ?>
	                    ...
	                    <div class="btn-group ">
	                        ...
	                        <ul class="dropdown-menu">
	                            <li>
	                                <a href="?Modulo=TransaccionesClinica&id=<?=$id_clinica?>&tipo=2&fecha1=<?=$fecha1?>&fecha2=<?=$fecha2?>"> Ingresos </a>
	                            </li>
	                            <li>
	                                <a href="?Modulo=TransaccionesClinica&id=<?=$id_clinica?>&tipo=1&fecha1=<?=$fecha1?>&fecha2=<?=$fecha2?>"> Gastos </a>
	                            </li>
	                            <li>
	                                <a href="?Modulo=TransaccionesClinica&id=<?=$id_clinica?>&fecha1=<?=$fecha1?>&fecha2=<?=$fecha2?>"> Todos </a>
	                            </li>
	                    ...
	                    <table class="table table-hover table-light">
	                        ...
	                        <tbody>
	                            ...
	                            <tr <? if($activo==0): echo "style='text-decoration:line-through;' class='danger' "; endif; ?>>
	                                ...
	                                <td class="font-dark"> <?=$operaciones[$val][2]?> </td>
	                                <td class="font-dark"> <? echo $operaciones[$val][3]; if($operaciones[$val][7]): echo "<br><small><em>".$operaciones[$val][7]."</em></small>"; endif;?> 
	                                							
	                                						<? if($operaciones[$val][9]==2){ echo "<br><small><em>PROVEEDOR: ".$operaciones[$val][1]."</em></small>"; }?> 
	                                						<? if($operaciones[$val][10]): echo "<br><small><em>".$fecha." ".datosCuenta($operaciones[$val][10])."</em></small>"; endif;?>
	                                </td>
	                            ...
	                            <? 
		                            if(!$_GET['fecha1'] && $cuenta==30): break; endif;
		                            $cuenta++;
		                            endforeach; ?>
	                            <!--
	                            <tr>
	                            ...
	                            <tr>
	                                ...
	                                <td class="font-dark">  </td>
	                                <td class="font-dark">  </td>
	                                <td align="right" class="font-dark"> <strong> <?=number_format($total_ingresos,2)?> </strong></td>
	                                <td align="right" class="font-dark"> <strong> <?=number_format($total_egresos,2)?> </strong> </td>
...
function eliminaOperacion(id,tipo,id_consulta){
	if(id_consulta==0){
		...
		var mensaje = "Al eliminar este movimiento se eliminará la consulta";
	}
	swal({
		title: "Eliminar operación",
		text: "¿Estás seguro que quieres eliminar la operación? <br><b>"+mensaje+"</b>",
		type: "warning",
	    ...
	    if(datos[0]==1){
			...
			confirmButtonText: "Ok",
				...
				window.open("?Modulo=TransaccionesClinica&id=<?=$id_cuenta_get?>&tipo=<?=$tipo_web?>&fecha1=<?=$fecha1?>&fecha2=<?=$fecha2?>", "_self");
			});
			}else{
...
function cambiaFecha(){
	...
	$('#load').show();
	if((fecha1)&&(fecha2)){
		window.open("?Modulo=TransaccionesClinica&id=<?=$id_clinica?>&fecha1="+fecha1+"&fecha2="+fecha2, "_self");
	}else{
