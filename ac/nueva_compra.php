...
$fecha=fechaBase3($fecha);


	//Insertamos datos
	$sql="INSERT INTO books_gastos (id_cuenta,id_proveedor,id_usuario,id_metodo_pago,id_tipo_gasto,fecha_hora_captura,fecha_gasto,monto,referencia,id_pago_especialistas_lab) VALUES ('$id_cuenta','$id_proveedor','$s_id_usuario','$id_metodo_pago','$id_tipo_gasto','$fechahora','$fecha','$monto','$descripcion','$id_pago_especialista')";
	$q=mysql_query($sql);
	...
	if($q){
		if($liquidado==1){
			$sq="UPDATE pagos_especialistas_lab SET liquidado=1 WHERE id_pago_especialistas_lab=$id_pago_especialista";
			$q=mysql_query($sq);
		}
		echo "1|$imprimir";
	}else{
