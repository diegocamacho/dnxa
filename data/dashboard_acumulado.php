...
<div class="row">
						<?foreach($empresas as $empresa){
							...
							$pacientes_clinica = mysql_query("SELECT DISTINCT(id_cita),id_paciente FROM citas 
															WHERE id_clinica = '$id_clinica' AND DATE(fecha_hora) >= '$inicial' AND DATE(fecha_hora) <= '$final'  AND citas.tipo=1 AND citas.activo=1 AND citas.atendida = 1");
							$pacientes_atendidos = mysql_num_rows($pacientes_clinica);
