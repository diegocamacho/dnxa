<?
$sql="SELECT * FROM books_tipos_ingreso ORDER BY eliminable DESC, cuenta_ingreso ASC";
$q=mysql_query($sql);
$cuentas = array();
			...
			<div class="portlet light  portlet-fit">
				<div class="portlet-title">
					<div class="caption">
						...
						<span class="caption-subject font-dark bold uppercase">Cuentas de ingresos</span>
					</div>
				...
				<div class="portlet-body">
					...
					<table class="table table-striped table-bordered table-hover">
					    ...
					    <tbody>
					        ...
					        <tr>
								<td><?=$cuenta->cuenta_ingreso?></td>
								<td align="right">
									<? if($cuenta->eliminable==1): ?><!-- Solo se van a poder eliminar y editar las cuentas que seleccione el usuario -->
										<img src="assets/global/img/loading-spinner-grey.gif" border="0" id="load_<?=$cuenta->id_tipo_ingreso?>" width="19" class="oculto" />
										<? if($cuenta->activo==1): ?>
											<a role="button" class="btn green btn-xs btn_<?=$cuenta->id_tipo_ingreso?>" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#EditaCuenta" data-id="<?=$cuenta->id_tipo_ingreso?>">Editar</a>
											<a role="button" class="btn red btn-xs btn_<?=$cuenta->id_tipo_ingreso?>" onclick="javascript:Desactiva(<?=$cuenta->id_tipo_ingreso?>)">Desactivar</a>
										<? else: ?>
											<a role="button" class="btn btn-warning btn-xs btn_<?=$cuenta->id_tipo_ingreso?>" onclick="javascript:Activa(<?=$cuenta->id_tipo_ingreso?>)">Activar</a>
										<? endif; ?>
									<? else: ?>
					...
					<div class="alert alert-dismissable alert-warning">
				  		<button type="button" class="close" data-dismiss="alert">×</button>
				  		<p>Aún no se han creado cuentas de ingresos</p>
				  	</div>
					<? endif; ?>
				</div>
			</div>
