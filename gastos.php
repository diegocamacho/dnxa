<?
$sql="SELECT * FROM clinicas WHERE activo=1 AND id_clinica=$s_id_clinica";
$q=mysql_query($sql);
...
$consulta="AND id_empresa=$s_id_clinica ";
//Cuentas
...
WHERE books_cuentas.activo=1 $consulta ORDER BY books_cuentas.id_empresa ASC";
$q=mysql_query($sql);
$cuentas = array();
while($datos=mysql_fetch_object($q)):
	$cuentas[] = $datos;
	        ...
	        <div class="portlet light portlet-fit ">
	            <div class="portlet-title">
	                ...
	                <div class="actions">
						...
						<a href="javascript:;" role="button" class="btn blue-chambray" data-toggle="modal" data-backdrop="static" data-keyboard="false" data-target="#nuevaCompra"> Nuevo Gasto </a>	                    
		                        ...
		                        <? foreach($cuentas as $cuenta): 
			                        ...
			                        if($cambio!=$cuenta->id_empresa):
			                        	echo '<tr>
												...
												<td align="right"> <a href="?Modulo=TransaccionesClinica" role="button" class="btn  green-jungle btn-xs ">Todas</a>	 </td>
											</tr>';
			                        endif;
	...
	$.post('ac/nueva_compra.php',datos,function(data){		
	    ...
	    if(datos[0]==1){
			...
			if(datos[1]){
				...
				window.open("?Modulo=Gastos&msg=1", "_self");
...
function guardaIngreso(){
	...
	$.post('ac/nuevo_ingreso.php',datos,function(data){
	    if(data==1){
			window.open("?Modulo=Gastos&msg=3", "_self");
	    }else{
	    	$('#load3').hide();
			$('.btn_ac').show();
...
function transferencia(){
	...
	$.post('ac/transferencia.php',datos,function(data){
	    if(data==1){
			window.open("?Modulo=Gastos&msg=2", "_self");
	    }else{
	    	$('#load2').hide();
			$('.btn_ac').show();
